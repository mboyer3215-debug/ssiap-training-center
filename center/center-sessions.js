...
(function () {
    ...
    function _showLoader() { const l = document.getElementById('loader'); if (l) l.style.display = 'flex'; }
    function _hideLoader() { const l = document.getElementById('loader'); if (l) l.style.display = 'none'; }
    ...
    function displaySessions(sessionsList) {
        ...
        sessionsList.forEach(session => {
            ...
            const dateFin   = session.dateFin   ? new Date(session.dateFin).toLocaleDateString('fr-FR')   : '—';
                    ...
                    <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:10px;">
                        <div onclick="openSession('${session.sessionId}')" style="flex:1;min-width:200px;">
                            <div style="display:flex;align-items:center;gap:12px;margin-bottom:8px;">
                                <span style="background:${color}20;color:${color};padding:4px 10px;border-radius:20px;font-size:12px;font-weight:700;">${status.toUpperCase()}</span>
                            ...
                            <h3 style="margin:0 0 8px;color:#1f2937;font-size:18px;">${session.titre || 'Sans titre'}</h3>
        ...
        try {
            ...
            if (!session.stagiaires || session.stagiaires.length === 0) {
                ...
                session.stagiaires.forEach(s => {
                    const stagStatus = s.status || 'actif';
                    const statusBadge = stagStatus === 'actif'
    ...
    function openSessionModal(sessionId) {
        ...
        const dateDebutVal = session && session.dateDebut ? new Date(session.dateDebut).toISOString().split('T')[0] : today;
        const dateFinVal   = session && session.dateFin   ? new Date(session.dateFin).toISOString().split('T')[0]   : defaultEnd;
    ...
    async function createSession(data) {
        ...
        try {
            ...
            if (result.success || result.sessionId) {
                ...
                alert('Erreur : ' + (result.error || result.message || 'Création impossible'));
            }
        } catch (e) {
            console.error('Erreur createSession:', e);
    ...
    async function updateSession(sessionId, data) {
        ...
        try {
            ...
            if (result.success || result.sessionId) {
                ...
                alert('Erreur : ' + (result.error || result.message || 'Mise à jour impossible'));
            }
        } catch (e) {
            console.error('Erreur updateSession:', e);
    ...
    async function createStagiaire(sessionId, data) {
        ...
        try {
            ...
            console.error('Erreur createStagiaire:', e);
    ...
    function displayQRCode(stagiaireId) {
        ...
        qrWindow.document.write(`
            ...
            <body>
                <div class="card">
                    ...
                    <div class="info">Valide jusqu'au ${stagiaire.dateFin ? new Date(stagiaire.dateFin).toLocaleDateString('fr-FR') : '—'}</div>
                    <div id="qrcode"></div>
        ...
        qrWindow.onload = () => {
            new qrWindow.QRCode(qrWindow.document.getElementById('qrcode'), {
                text: stagiaire.qrCodeData || stagiaire.pin,
