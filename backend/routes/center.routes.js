...
// POST /api/center/login — brute-force + génère JWT
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (!centerData?.auth?.passwordHash) return res.status(401).json({ success: false, error: "Compte non initialisé, contactez l'administrateur" });
    ...
    if (centerData.status === 'inactive') return res.status(403).json({ success: false, error: "Compte désactivé, contactez l'administrateur" });
...
router.post('/forgot-password', async (req, res) => {
  ...
  try {
    ...
    try {
      const mailer = getMailer();
      await mailer.sendMail({ from: `"SSIAP Training" <${process.env.SMTP_USER}>`, to: email, subject: '🔑 Réinitialisation mot de passe', html: `<p>Cliquez ici pour réinitialiser : <a href="${resetUrl}">${resetUrl}</a></p><p>Lien valable 1 heure.</p>` });
...
// GET /api/center/list (admin uniquement via son propre JWT admin — à sécuriser côté admin routes)
router.get('/list', async (req, res) => {
  try {
    ...
    snapshot.forEach(child => {
      ...
      centers.push({
        centerId: child.key,
        info: { nom: c.info?.nom || '—', email: c.info?.email || c.auth?.email || '—', telephone: c.info?.telephone || '', ville: c.info?.ville || '', createdAt: c.info?.createdAt || null },
        license: { type: c.license?.type || 'DEMO', expiresAt: c.license?.expiresAt || null, maxFormateurs: c.license?.maxFormateurs || 1, maxStagiaires: c.license?.maxStagiaires || 10, activatedAt: c.license?.activatedAt || null },
        stats: c.stats || {}, status: c.status || 'active', lastLogin: c.auth?.lastLogin || null
      });
    });
