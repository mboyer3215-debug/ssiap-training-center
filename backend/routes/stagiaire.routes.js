...
async function generateUniquePIN() {
    let pin, exists = true, attempts = 0;
    while (exists && attempts < 100) {
        ...
        exists = Object.values(allCenters).some(center =>
            Object.values(center.stagiaires || {}).some(s => s.pin === pin)
        );
        attempts++;
    }
    return pin;
...
router.post('/create', async (req, res) => {
    try {
        ...
        if (!centerId || !sessionId || !nom || !prenom)
            return res.status(400).json({ success: false, error: 'centerId, sessionId, nom et prenom sont requis' });
        ...
        if (isLicenseExpired(centerData)) return res.status(403).json({ success: false, error: 'Licence expirée', licenseExpired: true });
        ...
        const stagiairesActifs   = Object.values(stagiairesSnapshot.val() || {}).filter(s => s.status === 'actif');
        const maxStagiaires      = centerData.license?.maxStagiaires || 999;
        if (stagiairesActifs.length >= maxStagiaires)
            return res.status(403).json({ success: false, error: `Limite atteinte : ${maxStagiaires} stagiaires actifs maximum` });
        ...
        const stagiaireData = {
            stagiaireId, centerId, sessionId,
            nom: nom.toUpperCase(), prenom, email: email || '', telephone: telephone || '',
            pin, qrCodeData, status: 'actif',
            dateDebut: session.dateDebut, dateFin: session.dateFin, createdAt: Date.now()
        };
        await db.ref(`centers/${centerId}/stagiaires/${stagiaireId}`).set(stagiaireData);
        await db.ref(`centers/${centerId}/sessions/${sessionId}/nbStagiaires`).transaction(count => (count || 0) + 1);
        console.log(`✅ Stagiaire créé: ${nom} ${prenom} - PIN: ${pin}`);
...
router.get('/list/:centerId', async (req, res) => {
    try {
        ...
        let stagiaires = Object.values(snapshot.val() || {});
        if (sessionId) stagiaires = stagiaires.filter(s => s.sessionId === sessionId);
        const now = Date.now();
        stagiaires = stagiaires.map(s => ({ ...s, status: s.dateFin && now > s.dateFin ? 'expiré' : 'actif' }));
...
router.put('/update/:stagiaireId', async (req, res) => {
    try {
        ...
        if (!(await stagRef.once('value')).exists()) return res.status(404).json({ success: false, error: 'Stagiaire introuvable' });
...
router.delete('/delete/:stagiaireId', async (req, res) => {
    try {
        ...
        if (!centerId) return res.status(400).json({ success: false, error: 'centerId requis' });
        const stagRef   = db.ref(`centers/${centerId}/stagiaires/${stagiaireId}`);
        const stagiaire = (await stagRef.once('value')).val();
        if (!stagiaire) return res.status(404).json({ success: false, error: 'Stagiaire introuvable' });
        if (stagiaire.sessionId)
            await db.ref(`centers/${centerId}/sessions/${stagiaire.sessionId}/nbStagiaires`).transaction(count => Math.max(0, (count || 1) - 1));
...
router.post('/login', async (req, res) => {
    try {
        const { centerId, pin, qrCode, stagiaireId } = req.body;
        let stagiaire = null, centerData = null, resolvedCenterId = centerId;

        if (!centerId && pin) {
            // Recherche globale par PIN
            ...
            for (const [cId, center] of Object.entries(allCenters)) {
                const found = Object.values(center.stagiaires || {}).find(s => s.pin === pin);
                if (found) { stagiaire = found; centerData = center; resolvedCenterId = cId; break; }
            }
            if (!stagiaire) return res.status(401).json({ success: false, error: 'Code PIN incorrect' });
        } else {
            if (!centerId) return res.status(400).json({ success: false, error: 'centerId requis' });
            const centerSnapshot = await db.ref(`centers/${centerId}`).once('value');
            ...
            if (qrCode) {
                try {
                    ...
                    stagiaire = (await db.ref(`centers/${centerId}/stagiaires/${decoded.stagiaireId}`).once('value')).val();
                } catch(e) { return res.status(400).json({ success: false, error: 'QR code invalide' }); }
            } else if (stagiaireId) {
                stagiaire = (await db.ref(`centers/${centerId}/stagiaires/${stagiaireId}`).once('value')).val();
                if (stagiaire && pin && stagiaire.pin !== pin) stagiaire = null;
            } else if (pin) {
                const snap = await db.ref(`centers/${centerId}/stagiaires`).orderByChild('pin').equalTo(pin).once('value');
        ...
        if (stagiaire.dateFin && Date.now() > stagiaire.dateFin)
            return res.status(403).json({ success: false, error: 'Accès expiré - session terminée' });
        ...
        res.json({
            success: true,
            stagiaire: {
                ...stagiaire, centerId: resolvedCenterId,
                session: sessionInfo ? {
                    titre: sessionInfo.titre, niveau: sessionInfo.niveau,
                    dateDebut: sessionInfo.dateDebut, dateFin: sessionInfo.dateFin
                } : null
            },
...
router.post('/save-result', async (req, res) => {
    try {
        ...
        await db.ref(`centers/${centerId}/stagiaires/${stagiaireId}/historique/${resultId}`).set({
            resultId, sessionId: sessionId || '',
            score: score || 0, total: total || 0, pct: pct || 0,
            niveau: niveau || 1, partieId: partieId || 'toutes', temps: temps || 0,
            mode: 'entrainement', date: timestamp, completedAt: timestamp,
            detailsEchecs: detailsEchecs || null, themesBreakdown: themesBreakdown || null,
        });
 ...
 * Lit depuis centers/.../stagiaires/.../historique
 * + fallback depuis results/{centerId}/{stagiaireId}
 */
router.get('/historique/:stagiaireId', async (req, res) => {
    try {
        ...
        // Chemin 1 : historique dans le nœud stagiaire (sauvegardé par save-result)
        const snap = await db.ref(`centers/${centerId}/stagiaires/${stagiaireId}/historique`).once('value');
        let historique = Object.values(snap.val() || {})
            .sort((a, b) => (b.date || b.completedAt || 0) - (a.date || a.completedAt || 0));

        // Chemin 2 : fallback depuis results/{centerId}/{stagiaireId}
        if (historique.length === 0) {
            const snapResults = await db.ref(`results/${centerId}/${stagiaireId}`).once('value');
            if (snapResults.exists()) {
                historique = Object.entries(snapResults.val())
                    .map(([sessionId, d]) => ({
                        resultId: sessionId, sessionId,
                        score: d.score || 0, total: d.total || 0,
                        pct: parseFloat(d.percentage) || 0,
                        niveau: d.niveau || 1, partieId: d.partieId || 'toutes',
                        temps: d.temps || 0,
                        date: d.completedAt || 0, completedAt: d.completedAt || 0,
                        mode: 'entrainement', detailsEchecs: null,
                    }))
